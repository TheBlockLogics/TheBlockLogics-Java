...
public class BlockPane extends RelativeLayout {
    ...
    if (this.nearestTarget == null) {
      ...
      if (var1.isReporter) {
        ((BlockBase) this.nearestTarget[1])
            .parentBlock.replaceArgWithBlock((BlockBase) this.nearestTarget[1], var5);
      } else {
  ...
  public Block getHitBlock(float f, float f2) {
    ...
    for (int i = INSERT_NORMAL; i < getChildCount(); i += INSERT_ABOVE) {
      ...
      if ((childAt instanceof Block) && childAt != this.root) {
        ...
        if (f > ((float) iArr[INSERT_NORMAL])
            && f < ((float) (iArr[INSERT_NORMAL] + block.getWidth()))
            && f2 > ((float) iArr[INSERT_ABOVE])
            && f2 < ((float) (iArr[INSERT_ABOVE] + block.getHeight()))) {
  ...
  public boolean hitTest(float f, float f2) {
    getLocationOnScreen(this.posArea);
    return f > ((float) this.posArea[INSERT_NORMAL])
        && f < ((float) (this.posArea[INSERT_NORMAL] + getWidth()))
        && f2 > ((float) this.posArea[INSERT_ABOVE])
        && f2 < ((float) (this.posArea[INSERT_ABOVE] + getHeight()));
  }
